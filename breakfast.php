...
<head>
    ...
    <title>Alashibarecipes - Breakfast Recipes</title>
...
<div class="container-fluid p-0">
        ...
        <div class="recipe-header w-100 position-relative">
            <img src="images/pancake.jpg" alt="Breakfast Image" class="img-fluid w-100" style="height: 400px; object-fit: cover;">
    ...
    <div class="container mt-5">
        <div class="recipe-content mt-4">
            <p>Start your morning with our collection of tasty breakfast recipes. From pancakes to scrambled eggs, find the perfect recipe to kickstart your day.</p>

            <!-- Display approved breakfast recipes -->
            <div class="row">
                <?php
                // Fetch approved breakfast recipes and images from the database
                $stmt = $pdo->query('
                    SELECT Breakfast.BreakfastID, Breakfast.BreakfastName, Breakfast.BreakfastDescription, Images.ImagePath 
                    FROM Breakfast 
                    JOIN Images ON Breakfast.ImageID = Images.ImageID
                    WHERE Breakfast.Status = "approved"
                ');
                ...
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            <a href="recipe.php?id=<?php echo $row['BreakfastID']; ?>&type=breakfast">
                                <img src="<?php echo htmlspecialchars($row['ImagePath']); ?>" class="card-img-top" alt="<?php echo htmlspecialchars($row['BreakfastName']); ?>">
                            </a>
                            <div class="card-body">
                                <h5 class="card-title">
                                    <a href="recipe.php?id=<?php echo $row['BreakfastID']; ?>&type=breakfast">
                                        <?php echo htmlspecialchars($row['BreakfastName']); ?>
                                    </a>
                                </h5>
                                <p class="card-text"><?php echo htmlspecialchars($row['BreakfastDescription']); ?></p>
                            </div>
                        </div>
                    </div>
                    <?php
