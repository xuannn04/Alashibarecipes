...
<head>
    ...
    <title>Alashibarecipes - Dinner Recipes</title>
    <link href="stylesindex.css" rel="stylesheet" type="text/css">
    <!-- Bootstrap CSS -->
    <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
    <!-- Custom Styles -->
    <link href="stylesdinner.css" rel="stylesheet" type="text/css">
...
<div class="container-fluid p-0">
        ...
        <div class="recipe-header w-100 position-relative">
            <img src="images/Oyakodon.jpg" alt="Dinner Image" class="img-fluid w-100" style="height: 400px; object-fit: cover;">
    ...
    <div class="container mt-5">
        <div class="recipe-content mt-4">
            <p>Explore our collection of delicious dinner recipes. From comforting classics to exciting new dishes, find the perfect recipe to end your day on a delicious note.</p>

            <!-- Display approved dinner recipes -->
            <div class="row">
                <?php
                // Fetch approved dinner recipes and images from the database
                $stmt = $pdo->query('
                    SELECT Dinner.DinnerID, Dinner.DinnerName, Dinner.DinnerDescription, Images.ImagePath 
                    FROM Dinner 
                    JOIN Images ON Dinner.ImageID = Images.ImageID
                    WHERE Dinner.Status = "approved"
                ');
                ...
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            <a href="recipe.php?id=<?php echo $row['DinnerID']; ?>&type=dinner">
                                <img src="<?php echo htmlspecialchars($row['ImagePath']); ?>" class="card-img-top" alt="<?php echo htmlspecialchars($row['DinnerName']); ?>">
                            </a>
                            <div class="card-body">
                                <h5 class="card-title">
                                    <a href="recipe.php?id=<?php echo $row['DinnerID']; ?>&type=dinner">
                                        <?php echo htmlspecialchars($row['DinnerName']); ?>
                                    </a>
                                </h5>
                                <p class="card-text"><?php echo htmlspecialchars($row['DinnerDescription']); ?></p>
                            </div>
                        </div>
                    </div>
                    <?php
