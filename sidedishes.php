...
<head>
    ...
    <title>Alashibarecipes - Side Dishes Recipes</title>
    <link href="stylesindex.css" rel="stylesheet" type="text/css">
    <!-- Bootstrap CSS -->
    <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
    <!-- Custom Styles -->
    <link href="stylessidedishes.css" rel="stylesheet" type="text/css">
...
<div class="container-fluid p-0">
    <!-- Recipe Image Section (Full width with overlay) -->
    <div class="recipe-header w-100 position-relative">
        <img src="images/Kimchi.jpg" alt="Side Image" class="img-fluid w-100" style="height: 400px; object-fit: cover;">
...
<div class="container mt-5">
    <div class="recipe-content mt-4">
        <p>Discover our collection of delicious side dish recipes. From crispy roasted vegetables to creamy mashed potatoes, find the perfect accompaniment to your main course.</p>

        <!-- Fetch and display approved side dish recipes -->
        <div class="row">
            <?php
            // Fetch approved side dish recipes and images from the database
            $stmt = $pdo->query('
                SELECT Side.SideID, Side.SideName, Side.SideDescription, Images.ImagePath 
                FROM Side 
                JOIN Images ON Side.ImageID = Images.ImageID
                WHERE Side.Status = "approved"
            ');
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                <div class="col-md-4">
                    <div class="card">
                        <a href="recipe.php?id=<?php echo $row['SideID']; ?>&type=side">
                            <img src="<?php echo htmlspecialchars($row['ImagePath']); ?>" class="card-img-top" alt="<?php echo htmlspecialchars($row['SideName']); ?>">
                        </a>
                        <div class="card-body">
                            <h5 class="card-title">
                                <a href="recipe.php?id=<?php echo $row['SideID']; ?>&type=side">
                                    <?php echo htmlspecialchars($row['SideName']); ?>
                                </a>
                            </h5>
                            <p class="card-text"><?php echo htmlspecialchars($row['SideDescription']); ?></p>
                        </div>
                    </div>
                </div>
                <?php
